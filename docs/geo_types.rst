...
some simple representative types in :mod:`dse.util` for inserting and retrieving data::
    ...
    session.execute("INSERT INTO ks.geo (k, point, circle, line, poly) VALUES (%s, %s, %s, %s)",
                    0, Point(1, 2), Circle(1, 2, 3), LineString(((1, 2), (3, 4))), Polygon(((1, 2), (3, 4), (5, 6))))
...
For prepared statements, shapely geometry types can be used interchangeably with the build-in types because their
...
In order to use shapely types in a CQL interpolated query, one must update the encoder with those types, specifying
the same string encoder as set for the internal types::

    from dse import util
    from shapely.geometry import Point, LineString, Polygon

    encoder_func = session.encoder.mapping[util.Circle]
...
Note that there is no shapely equivalent for :class:`.util.Circle` because that is a DSE contrived type, not
one of the standard OSC types.
